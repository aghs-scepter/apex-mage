...
class AsyncChannelRepository(Protocol):
    """Async protocol for channel persistence operations.

    Use this protocol for async implementations (e.g., SQLite with asyncio).
    """
...
class AsyncVendorRepository(Protocol):
    """Async protocol for vendor persistence operations.
...
class AsyncMessageRepository(Protocol):
    """Async protocol for message persistence operations.
...
class AsyncRateLimitRepository(Protocol):
    """Async protocol for rate limiting data operations.
    ...
    async def get_recent_text_request_count(
        ...
        Args:
            channel_external_id: The external platform ID for the channel.
            vendor_name: The vendor name to filter by (e.g., "Anthropic").
    ...
    async def get_recent_image_request_count(
        ...
        Args:
            channel_external_id: The external platform ID for the channel.
            vendor_name: The vendor name to filter by (e.g., "Fal.AI").
